...
Collected medical data used to improve patient care
...
* Activate the environment using the following command in a terminal window:
  ...
  * Windows: `activate mimic`
  * Linux, OS X: `source activate mimic`
...
* Deactivate the environment using the following command in a terminal window:
  ...
  * Windows: `deactivate mimic`
  * Linux, OS X: `source deactivate mimic`
               
* Delete the environment using the command (can't be undone):

  * `conda remove --name mimic --all`
...
│   └── py36       <- Example python package - place shared code in such a package
